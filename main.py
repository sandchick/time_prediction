...
for i, chip_id in enumerate(chip_ids):
    ...
    axes[i].plot(np.array(range(len(predict_data_y_gause))), predict_data_y_gause, color = 'black', label = 'evaluation result after filiter', linewidth = linewidth)
...
plt.savefig('../img/svr_test'+ str(predict_range) + '+' + str(threshold_AF) +'.png', bbox_inches = 'tight')
