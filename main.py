...
for k, predict_range in enumerate(predict_ranges):
    axes[k].bar(np.array(range(len(chip_ids))), RE_mean_array[k], color = 'red')
...
for i, chip_id in enumerate(chip_ids):
    axes[i].bar(np.array(range(len(predict_ranges))), RE_mean_array[:,i], color = 'red')
    axes[k].set_title('Chip = ' + str(i), fontsize = stitlesz)
    axes[k].set_xlabel('Predict index', fontsize = labelsz, labelpad = 20)
axes[0].set_ylabel('Relative Error ', fontsize = labelsz, labelpad = 30)
