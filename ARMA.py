...
for u,chip_id in enumerate(chip_ids):
# 读取数据集
    data_range = 0.85
    ...
    # 绘制预测结果和实际值
                ...
                plt.tight_layout()
                plt.savefig('../img/ARMA/kalmanpredict/' + str(chip_id) + '/' + str(p) + str(d) + str(q) + 'ARMA_pre.png')
                axes.cla()
                labels = axes.get_xticklabels() + axes.get_yticklabels()
                [label.set_fontsize(ticklabelsz) for label in labels]
    ...
    for i, p in enumerate(ps):
        for j, d in enumerate(ds):
            ...
            #error analsis
                ...
                err = np.array(err)
                avg_err = np.mean(err)
                f = open('../img/ARMA/' + str(train_range) + '/chip' + str(chip_id) + '/' + str(p) + str(d) + str(q) + 'ARMA_error.txt',"wt")
                print (err,file = f)
            ...
            #draw error
                ...
                plt.savefig('../img/ARMA/kalmanpredict/' + str(chip_id) + '/' + str(p) + str(d) + str(q) + 'ARMA_error.png')
                axes.cla()
                labels = axes.get_xticklabels() + axes.get_yticklabels()
                [label.set_fontsize(ticklabelsz) for label in labels]